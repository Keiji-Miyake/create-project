...
- AIアシスタントの基本動作とコミュニケーション方針の定義
- プロジェクト品質とドキュメント管理の標準化
- チーム開発の効率化と一貫性の確保
- セッション間のコンテキスト維持の実現
...
- コード品質の基準
  ...
  - "ファイル修正時のコード完全性"
    - write_to_fileツール使用時は必ず完全なファイル内容を提供
    - 「残りのコードは変更なし」などの省略は禁止
    - 変更箇所以外も含めて全コードを提供
    - インポート文から末尾まで漏れなく含める
...
- "switch_mode"ツールを使用
  - Plan mode: `<switch_mode><mode_slug>plan</mode_slug></switch_mode>`
  - Act mode: `<switch_mode><mode_slug>act</mode_slug></switch_mode>`
- 切り替えの判断基準
  - 新規タスク開始時: Plan modeで開始
  - 計画完了・実装開始時: Act modeへ切り替え
  - 計画の見直しが必要な場合: Plan modeへ戻る
...
- "タスク完了時の処理"
  ...
  - "コミット要否の判断"
    ...
    - 設定やドキュメントの更新時は必須
    - 動作確認が完了した場合のみコミット可能
  - "コミットメッセージの提案（プレフィックスと日本語説明）"
  - "git add/commitコマンドの提示"
...
- "仮定と不確実性の明確な共有"
- "コミットメッセージのルール"
  - コミットメッセージには人格や口調を含めない
  - プレフィックスと簡潔な日本語説明文で構成
  - プレフィックス例: feat:, fix:, docs:, style:, refactor:, test:, chore:
